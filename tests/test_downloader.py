...
class TestAttachmentDownloader(unittest.TestCase):
    def setUp(self):
        ...
        self.config = {
            ...
            "enabled_categories": ["Bills", "Other"],
        }

    def tearDown(self):
        shutil.rmtree(self.test_dir)

    @patch("downloader.AttachmentClassifier")
    @patch("downloader.MailBox")
    def test_process_single_account(self, mock_mailbox_cls, mock_classifier_cls):
        ...
        mock_msg = MagicMock()
        mock_msg.uid = "100"
        mock_msg.subject = "Electric Bill"
        ...
        # Run processing - now uses date object
        self.downloader.process_single_account(
            self.config["accounts"][0], date(2023, 10, 1)
        )

        # Verify file downloaded - new structure: Category/Year/Month/filename
        expected_path = Path(self.test_dir) / "Bills" / "2023" / "10" / "invoice.pdf"
    ...
    @patch("downloader.AttachmentClassifier")
    @patch("downloader.MailBox")
    def test_filter_extension(self, mock_mailbox_cls, mock_classifier_cls):
        mock_classifier = mock_classifier_cls.return_value
        mock_classifier.classify.return_value = {
            "category": "Other",
            "reasoning": "Test",
        ...
        # File should NOT exist (exe not in allowed_extensions)
        expected_path = Path(self.test_dir) / "Other" / "2023" / "10" / "virus.exe"
    ...
    @patch("downloader.AttachmentClassifier")
    @patch("downloader.MailBox")
    def test_duplicate_filename(self, mock_mailbox_cls, mock_classifier_cls):
        mock_classifier = mock_classifier_cls.return_value
        mock_classifier.classify.return_value = {
            "category": "Bills",
            "reasoning": "Test",
        ...
        # Force create a file that would collide - new structure
        target_dir = Path(self.test_dir) / "Bills" / "2023" / "10"
